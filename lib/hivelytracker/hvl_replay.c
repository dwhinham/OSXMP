...
bool hvl_InitSubsong( struct hvl_tune *ht, uint32_t nr )
{
  uint32_t PosNr, i;
...
struct hvl_tune *hvl_load_ahx( const uint8_t *buf, size_t buflen, uint32_t defstereo, uint32_t freq )
{
  const uint8_t  *bptr;
  const char     *nptr;
  ...
  strncpy( ht->ht_Name, (char *)&buf[(buf[4]<<8)|buf[5]], 128 );
  nptr = (char *)&buf[((buf[4]<<8)|buf[5])+strlen( ht->ht_Name )+1];
...
struct hvl_tune *hvl_load_hvl( const uint8_t *buf, size_t buflen, uint32_t defstereo, uint32_t freq )
{
  const uint8_t  *bptr;
  const char     *nptr;
...
struct hvl_tune *hvl_ParseTune( const uint8_t *buf, size_t buflen, uint32_t freq, uint32_t defstereo )
{
...
struct hvl_tune *hvl_LoadTune( const char *name, uint32_t freq, uint32_t defstereo )
{
  struct hvl_tune *ht = NULL;
  uint8_t  *buf;
  size_t  buflen;
...
void hvl_process_stepfx_1( struct hvl_tune *ht, struct hvl_voice *voice, int32_t FX, int32_t FXParam )
{
  switch( FX )
  {
...
void hvl_process_stepfx_2( const struct hvl_tune *ht, struct hvl_voice *voice, int32_t FX, int32_t FXParam, int32_t *Note )
{
  switch( FX )
  {
...
void hvl_process_stepfx_3( struct hvl_tune *ht, struct hvl_voice *voice, int32_t FX, int32_t FXParam )
{
  int32_t i;
...
void hvl_plist_command_parse( const struct hvl_tune *ht, struct hvl_voice *voice, int32_t FX, int32_t FXParam )
{
  switch( FX )
  {
    case 0:
...
{
  static const uint8_t Offsets[] = {0x00,0x04,0x04+0x08,0x04+0x08+0x10,0x04+0x08+0x10+0x20,0x04+0x08+0x10+0x20+0x40};
...
void hvl_set_audio( struct hvl_voice *voice, double freqf )
{
  if( voice->vc_TrackOn == 0 )
  {
...
void hvl_mixchunk( struct hvl_tune *ht, uint32_t samples, int8_t *buf1, int8_t *buf2, int32_t bufmod )
{
...
void hvl_DecodeFrame( struct hvl_tune *ht, int8_t *buf1, int8_t *buf2, int32_t bufmod )
{
